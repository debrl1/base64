 ...
 */
void encode_group(const uint8_t *input, size_t input_length, uint8_t *output) {
    output[0] = input[0] >> 2;
...
uint8_t *base64_encode(const uint8_t *input, size_t length) {
    ...
    // Calculating the length of the array that will be returned and allocating memory for it;
    size_t newlen = base64_elen(length);
    uint32_t k = 0;
    ...
    for (size_t i = 0; i < length; i += 3) {
        ...
        // Encoding the copied values and writing these values to the resulting array;
    ...
    // Turning the encoded values into printable characters and delimiting the base64 string;
