...
uint8_t *base64_decode(const uint8_t *input) {
    ...
    // Copying the encoded array and setting all base64 digits equal to their 6-bit values;
    size_t length = strlen((char *) input);
    ...
    // Calculating the length of the decoded string and allocating memory for it
    ...
    for (size_t i = 0; i < length; i += 4) {
        ...
        // Decoding the group and writing these decoded values to the array that will be returned
